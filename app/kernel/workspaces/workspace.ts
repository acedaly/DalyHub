...
/**
 * The shape a workspace identifier must take: a non-empty, bounded, portable
 * scope identifier. The charset is deliberately conservative — letters, digits,
 * hyphen and underscore — so ids are safe to use verbatim as lookup keys and
 * index keys, and so `crypto.randomUUID()` output (hex + hyphens) is always
 * valid. Workspace ids are SCOPE identifiers, not authentication secrets
 * (ADR-010): the pattern is about structural safety, not unguessability.
 */
 ...
 * raw string (e.g. configuration) into a `WorkspaceId`.
 */
