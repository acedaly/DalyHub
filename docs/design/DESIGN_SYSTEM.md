...
- **Escape & history:** Escape and the close button behave like browser Back — they close **only the top** level (unless `preventClose` is set) and are Back-aware so Forward can restore. Browser Back closes the top; Forward restores it; navigating to another page exits the stack.
