...
**Explicit save.** `useForm` for surfaces where commitment matters: clear **Save**/**Cancel**, dirty-state tracking, disabled/pending behaviour that **prevents duplicate submits**, keyboard submission, server errors that preserve the complete draft, a baseline reset on success, and a Cancel that restores the last committed value.
...
**Navigation safety.** While an explicit form is dirty, both in-app navigation (via `UnsavedChangesGuard` → an accessible confirm) and full-page unload (the browser prompt) are intercepted, so a draft is never silently discarded.
...
**Entity-link picker.** ONE entity-agnostic picker for creating and managing [FND-04 EntityLinks](../../AGENTS.md#95-entitylinks). It takes typed configuration (anchor entity, permitted target types, link-type descriptors, direction, single/multiple, an async workspace-scoped `searchTargets` loader, existing links, and `onLink`/`onUnlink` callbacks) and **never imports D1, bindings or repositories**. A server service ([`app/platform/entity-links`](../../app/platform/entity-links)) maps its operations to the existing FND-04 repository (workspace scope + Activity actor stay server-side, direction honoured, reserved spine types refused) — no second relationship model, no migration. It excludes the anchor from its own results, prevents duplicate active links, de-duplicates, bounds result sizes, handles deleted/stale/inaccessible targets calmly, keeps kernel identifiers while showing user-language labels, and never leaks an inaccessible entity's title. Its `searchTargets` contract lets [DS-08](../roadmap/ROADMAP_V2.md#-ds-08--shared-search) supply real search later without replacing the picker.
